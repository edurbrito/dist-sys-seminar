...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec auctor, nisl eget aliquam aliquam.
...
The base layer tooling choice resided in the \emph{Nuxt.js} framework, a meta-framework
for building web applications based on \emph{Vue.js}. With that established,
it became clear the need for a protocol for syncing and communicating in a 
peer-to-peer way between the devices. The choice resided in the \emph{GUN} ecosystem,
...
this was the language chosen for the interactive web development. The cryptographic standards used in the 
\emph{PoW} algorithm were imported from the \emph{crypto-js} library. And, finally, HTML and CSS, with the help
of the \emph{Tailwind CSS} framework were used for styling and presentation.
...
and that is based on \emph{Web Workers}. This \emph{API} was used in order to offload
...
\begin{figure*}[h]
  \centering
  \captionsetup{justification=centering}
  % \includegraphics{in-browser-pow-topology}
  \caption{blblb}
...
of them. This setting enforces higher synchrony between the peers, as there is no mechanism for
general chain data synchronization. This also means that a new peer joining the network does not
