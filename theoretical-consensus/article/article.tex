...
but also about their order - naturality forming the underlying structure of what is now known as a blockchain. 
...
that work in trusted environments \cite{castro1999practical}. However, even those protocols have
their own limitations, not only in terms of trust, fault-tolerance, centrality, permissioning,
or bottlenecking, but also in terms of scalability \cite{miller2016honey}. Trying to put some effort on decentralization, 
...
participating devices, without interrupting consensus and while generally battling Sybil attacks \cite{survey-dist-consensus}.
Fundamentally, the permissionless consensus problem is the need for a consensus protocol that
can be run in a "zero-trust" environment, where the participants are unknown and untrusted,
...
that depit lower connectivity between the participants. Operationally,
...
The goal of permissionless consensus is, as generally for any consensus protocol, to reach
...
networks, is by interchaining the generation of the agreeable value, 
...
and are replicated with some variations in the other permissionless networks.
...
when it comes to the generation of the block and its existential and verifiable proof.
